...
class ViewQuestionAdapter(val dlist: List<Quiz>, val listener: OnQuestionListner) :
    RecyclerView.Adapter<ViewQuestionAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        quiz.image?.let {
            holder.view.iv_questionitem.setImageBitmap(Helper.getIaeFromByteArray(it))
        }
        holder.view.iv_delete_questionitem.setOnClickListener({
            listener.onQuestionDelete(
                quiz.id
            )
        })
        holder.view.iv_edit_questionitem.setOnClickListener({
            listener.onQuestionEdit(
                quiz
            )
        })
